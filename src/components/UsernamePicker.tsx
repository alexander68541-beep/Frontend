...
export function UsernamePicker({ portfolio }: { portfolio: Portfolio }) {
  ...
  const canSave = !!normalized && !unchanged && !localError && check?.available === true;
    ...
    <div className="card">
      ...
      <div className="stack gap-2 mt-6">
        ...
        {!localError && !checking && check && !unchanged && (
          <span className={check.available ? "field-hint" : "field-error"} style={check.available ? { color: "var(--ok)" } : undefined}>
